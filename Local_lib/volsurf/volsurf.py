...
from Local_lib.volsurf.useful import raw_total_variance, get_crossings
...
class VolatilitySurface:
    ...
    def from_date(cls, date, filters=(), *args, **kwargs):
        ...
        path = f"Data/tickers/{date_str}.csv"
        ...
        df = df[~df['instrument_name'].str.startswith('BTC')]
    ...
    @property
    def svi_path(self):
        return f"Results/volsurf/svi/{self.name}.csv"
    ...
    def plot(self, fig=None, log_scale=True):
        ...
        for idx, ((ttm, df), ax) in enumerate(zip(groups, fig.get_axes())):
            ...
            ax.plot(ks, ivs, marker='o', c='red', linewidth=0)
